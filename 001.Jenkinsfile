...
def NODE_VERSION_DICT=["v12.22":"/root/node-v12.22.12-linux-x64/bin/", "v14.18":"/root/node-v14.18.0-linux-x64/bin/", "v16.13":"/root/node-v16.13.0-linux-x64/bin/"]
...
pipeline {
    ...
    stages {
        ...
        stage('Build BMS') {
            steps {
                //sh "[ \"${params.need_build}\" == \"true\" ] && cd BMS && rimraf node_modules"
                // sh "[ \"${params.need_build}\" == \"true\" ] && cd BMS && rm -rf ./node_modules"
                // sh "[ \"${params.need_build}\" == \"true\" ] && cd BMS && cnpm install && npm run build"
                // sh "[ \"${params.need_build}\" == \"true\" ] && cd BMS && rm -rf ./node_modules && npm install --registry=http://nexus.dev.datatist.cn/nexus/repository/npm-all/ && npm run build"
                ...
                if [[ ${params.need_build} == "true" ]];then
                    if [[ ${params.package_manager} == "npm" ]];then
                        ...
                        cd BMS && npm  install -g pnpm@7   --registry=https://registry.npmmirror.com && pnpm install --no-frozen-lockfile ${NPM_REGISTORY} && pnpm build
                    fi
                fi
                """
            }
            ...
            steps {
                ...
                if [[ ${params.need_build} == "true" ]];then
                    if [[ ${params.package_manager} == "npm" ]];then
                        ...
                        cd Front && npm  install -g pnpm@7  --registry=https://registry.npmmirror.com && pnpm install  --no-frozen-lockfile ${NPM_REGISTORY} && pnpm build
                    fi
                fi
        ...
        // stage('Build Front') {
        //     steps {
        //         sh "cd Front && rm -rf ./node_modules && cnpm install && cnpm run build"
        //     }
        // }
